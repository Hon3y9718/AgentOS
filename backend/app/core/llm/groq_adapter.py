"""Groq provider adapter (API_CONTRACT.md §5.5, ADR-0002).

Role: translates LLMRequest into a Groq Chat Completions API call and its
SSE stream into normalized LLMEvents. Groq's API is documented as
OpenAI-compatible, and this file is a deliberate near-duplicate of
openai_adapter.py rather than a shared base class (asked and confirmed —
each provider's file can freely diverge without touching a shared
abstraction, at the cost of the duplication this represents).
...
app.core.llm.types.
Gotcha: same content-block-boundary synthesis as openai_adapter.py — Groq's
stream never explicitly closes a block either.
Gotcha: verified live (not assumed) during implementation that Groq's usage
numbers arrive under the same top-level `"usage"` field OpenAI uses
(`prompt_tokens`/`completion_tokens`), on a final chunk with `"choices": []`
— NOT under the provider-specific `"x_groq"` field, which carries only
request metadata (queue time, request id), never the token counts.
...
_CODE_OVERRIDE_MAP: dict[str, type[DomainError]] = {
    ...
    # WHY model_not_found -> InvalidRequestError explicitly, even though
    # the fallback ("invalid_request_error" type) already lands there too:
    # verified live during implementation that Groq returns this exact code
    # for an unknown model, at HTTP 404 (not 400) — the override map makes
    # that intentional, not incidental.
    "model_not_found": InvalidRequestError,
}


class GroqAdapter:
    """ProviderAdapter for Groq's Chat Completions API. See adapter.py."""
        ...
        async with (
            ...
            async for chunk in _iter_sse_data(response):
                ...
                for tool_call in delta.get("tool_calls") or []:
                    groq_index = tool_call["index"]
                    if groq_index not in tool_call_indices:
                        norm_index = next_index
                        next_index += 1
                        tool_call_indices[groq_index] = norm_index
                    ...
                    norm_index = tool_call_indices[groq_index]
...
def _build_payload(request: LLMRequest) -> dict[str, Any]:
    ...
    for message in request.messages:
        messages.extend(_to_groq_messages(message))
    ...
    if request.tools:
        payload["tools"] = [_to_groq_tool(t) for t in request.tools]
        payload["tool_choice"] = _to_groq_tool_choice(request.tool_choice)
    return payload


def _to_groq_messages(message: LLMMessage) -> list[dict[str, Any]]:
    """One LLMMessage can expand into *multiple* Groq messages — same
    tool_result-becomes-its-own-message shape as OpenAI. See
    openai_adapter.py's identical function for the full reasoning."""
    if message.role == "user":
        ...
        for block in message.content:
            if isinstance(block, ToolResultBlock):
                ...
                content_parts.append(_to_groq_image_part(block))
            else:
...
def _to_groq_image_part(block: ImageBlock) -> dict[str, Any]:
    source = block.source
...
def _to_groq_tool(tool: ToolDefinition) -> dict[str, Any]:
    return {
        "type": "function",
        "function": {
            "name": tool.name,
...
def _to_groq_tool_choice(tool_choice: str | dict[str, str]) -> str | dict[str, Any]:
...
def _map_error(
    ...
    kwargs: dict[str, Any] = {
        "code": f"provider.groq.{code or error_type or 'unknown'}",
        "details": {"provider": "groq", "type": error_type, "code": code},
    }
    if retry_after_seconds is not None:
        kwargs["retry_after_seconds"] = retry_after_seconds
    return error_cls(message or f"Groq returned an unmapped error: {error_type}.", **kwargs)
