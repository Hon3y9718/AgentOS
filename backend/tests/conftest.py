...
Role: seeds a default DATABASE_URL/SECRET_KEY before app.main (and therefore
app.config) is imported, and provides a lifespan-aware test client plus a
real registered-and-logged-in user for authenticated-endpoint tests.
...
# `settings = Settings()` at module import time, so the env vars must exist
